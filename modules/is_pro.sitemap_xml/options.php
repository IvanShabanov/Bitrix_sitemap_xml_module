...
foreach ($siteIds as $sId => $sName) {
	...
	foreach ($options_list as $option_name => $arOption) {
			...
			if ($arOption['type'] == 'file') {
				...
				if (!empty($files) && (isset($files['tmp_name'])) && (!empty($files['tmp_name']))) {
					...
					if ($optionIsValid !== true) {
						$eeror_message .= 'ERROR: ' . Loc::getMessage('ISPRO_SITEMAP_XML_' . $option_name_def) . ' ' . $optionIsValid . PHP_EOL;
					}
				}
		...
		if (($saveOption || $setDefault) && ($optionIsValid === true)) {
			...
			$ok_message .= 'SAVED: ' . Loc::getMessage('ISPRO_SITEMAP_XML_' . $option_name_def) . PHP_EOL;
		};
...
$tabList[] = [
	'DIV' => 'description',
	'TAB' => Loc::getMessage('ISPRO_SITEMAP_XMLe_TAB_SET_DESC'),
	'ICON' => 'ib_settings',
		...
		<? foreach ($options_list as $option_name => $arOption) : ?>
			...
			<tr>
				<td width="20%" valign="top">
					<?= Loc::getMessage('ISPRO_SITEMAP_XML_' . $option_name_def) ?>
				</td>
						...
						<select name="option_<?= $option_name ?>">
							<? foreach ($arOption['values'] as $value) : ?>
								<option value="<?= $value ?>" <?= ($option[$option_name] == $value) ? 'selected' : '' ?>>
									<?= Loc::getMessage('ISPRO_SITEMAP_XML_' . $option_name_def . '_' . $value) ?>
								</option>
							<? endforeach ?>
						</select>
